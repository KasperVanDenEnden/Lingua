...
describe('UserDto Tests', () => {
    ...
    it('should fail validation when location is missing', async () => {
        ...
        expect(errors[0].constraints?.["isNotEmpty"]).toBe('role should not be empty');
    })
    ...
    it('should fail validation when firstname is missing', async () => {
        ...
        expect(errors[0].constraints?.["isNotEmpty"]).toBe('firstname should not be empty');
    })
    ...
    it('should fail validation when lastname is missing', async () => {
        ...
        expect(errors[0].constraints?.["isNotEmpty"]).toBe('lastname should not be empty');
    })
    ...
    it('should fail validation when location is not valid type', async () => {
        ...
        expect(errors.length).toBeGreaterThan(0);
        expect(errors[0].property).toBe('firstname');
        expect(errors[0].constraints?.["isString"]).toBe('firstname must be a string');
    })
    ...
    it('should fail validation when lastname is not valid type', async () => {
        ...
        expect(errors.length).toBeGreaterThan(0);
        expect(errors[0].property).toBe('lastname');
        expect(errors[0].constraints?.["isString"]).toBe('lastname must be a string');
    })
    ...
    it('should fail validation when role is not valid type', async () => {
        ...
        expect(errors.length).toBeGreaterThan(0);
        expect(errors[0].property).toBe('role');
        expect(errors[0].constraints?.["isEnum"]).toBe('Role must be a valid enum value');
    })
