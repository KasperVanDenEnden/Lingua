...
export class CourseAssistantService {
    ...
    addAssistant(assistant: Id, courseId: string): Observable<ICourse> {
        const data: IUpdateCourseAssistant = { assistant: assistant, course: courseId  };

        return this.http
            .post<ICourse>(`${environment.dataApiUrl}/assistant/add`, data , this.auth.getHttpOptions());
    }
    ...
    removeAssistant(assistant: Id, courseId: string): Observable<ICourse>  {
        ...
        return this.http
            .post<ICourse>(`${environment.dataApiUrl}/assistant/remove`, data, this.auth.getHttpOptions());
    }
}