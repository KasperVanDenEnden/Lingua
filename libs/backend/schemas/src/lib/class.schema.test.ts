...
describe('ClassSchema Tests', () => {
    ...
    it('should fail validation if title is missing', async () => {
        ...
        expect(errors[0].constraints?.['isNotEmpty']).toBe('title should not be empty');
    })
    ...
    it('should fail validation if status is missing', async () => {
        ...
        expect(errors[0].constraints?.['isNotEmpty']).toBe('status should not be empty');
    })
    ...
    it('should fail validation if createdOn is missing', async () => {
        ...
        expect(errors[0].constraints?.['isNotEmpty']).toBe('createdOn should not be empty');
    })
    ...
    it('should fail validation if assistants is missing', async () => {
        ...
        expect(errors[0].constraints?.['isNotEmpty']).toBe('assistants should not be empty');
    })
    ...
    it('should fail validation if createdOn is invalid type', async () => {
        ...
        expect(errors[0].constraints?.['isDate']).toBe('createdOn must be a Date instance');
    })
    ...
    it('should fail validation if language is invalid type', async () => {
        ...
        expect(errors[0].constraints?.['isEnum']).toBe('Language must be a valid enum value');
    })
    ...
    it('should fail validation if assistants is invalid type', async () => {
        ...
        expect(errors[0].constraints?.['arrayMinSize']).toBe('Assistants must be an array (can be empty)');
        expect(errors[0].constraints?.['isArray']).toBe('assistants must be an array');
        expect(errors[0].constraints?.['isObjectId']).toBe('Each asstisant must be a valid ObjectId');
    })
})