...
export class LessonAttendanceService {
    ...
    async subscribe(id: Id, studentId: Id): Promise<ILesson> {
        ...
        if (existingSubscription) throw new HttpException('Student already subscribed!', HttpStatus.BAD_REQUEST);
        ...
        if (!updatedLesson) throw new HttpException('Lesson not found', HttpStatus.NOT_FOUND);
    ...
    async unsubscribe(id: Id, studentId: Id): Promise<ILesson> {
        ...
        if (!existingLesson) throw new HttpException('Student not found as subscriber', HttpStatus.NOT_FOUND);
