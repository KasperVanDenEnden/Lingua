...
describe('LocationDto Tests', () => { 
    ...
    it('should fail validation when createdBy is missing', async() => {
        ...
        expect(errors[0].constraints?.["isNotEmpty"]).toBe('createdBy should not be empty');
    })
    ...
    it('should fail validation when street is missing', async() => {
        ...
        expect(errors[0].constraints?.["isNotEmpty"]).toBe('street should not be empty');
    })
    ...
    it('should fail validation when number is missing', async() => {
        ...
        expect(errors[0].constraints?.["isNotEmpty"]).toBe('number should not be empty');
    })
    ...
    it('should fail validation when createdBy is missing', async() => {
        ...
        expect(errors[0].constraints?.["isNotEmpty"]).toBe('city should not be empty');
    })
    ...
    it('should fail validation when postal is missing', async() => {
        ...
        expect(errors[0].constraints?.["isNotEmpty"]).toBe('postal should not be empty');
    })
    ...
    it('should fail validation when province is missing', async() => {
        ...
        expect(errors[0].constraints?.["isNotEmpty"]).toBe('province should not be empty');
    })
    ...
    it('should fail validation when createdBy is not valid type', async () => {
        ...
        expect(errors.length).toBeGreaterThan(0);
        expect(errors[0].property).toBe('createdBy');
        expect(errors[0].constraints?.["isObjectId"]).toBe('createdBy must be a valid ObjectId');
    })
    ...
    it('should fail validation when province is not valid type', async () => {
        ...
        expect(errors.length).toBeGreaterThan(0);
        expect(errors[0].property).toBe('province');
        expect(errors[0].constraints?.["isEnum"]).toBe('Province must be a valid enum value');
    })
})
